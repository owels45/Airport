...
/*
    ...
    public void build() {
        ...
        //service vehicle
        serviceVehicleOilPort = new ArrayList<>();
    }
    ...
    @Subscribe
    public void receive(ServiceVehicleStart serviceVehicleStart) {
        LogEngine.instance.write("+ Body.receive(" + serviceVehicleStart + ")");
        
        try {
            // ServiceVehicleOil: increaselevelApuOilTank
            for (int aPUOilTankIndex = 0; aPUOilTankIndex < aPUOilTanks.size(); aPUOilTankIndex++) {
                Method increaseLevelOfAPUOilTank = serviceVehicleOilPort.get(0).getClass().getDeclaredMethod("increaseLevelOfAPUOilTank", Object.class);
                LogEngine.instance.write("increaseLevelOfAPUOilTank = " + increaseLevelOfAPUOilTank);
                int increasedAmount = increaseLevelOfAPUOilTank.invoke(serviceVehicleOilPort.get(0), aPUOilTanks.get(aPUOilTankIndex));
            }
