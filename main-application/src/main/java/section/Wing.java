...
public class Wing extends Subscriber {
    ...
    public void receive(ServiceVehicleStart serviceVehicleStart) {
        ...
        try {
            // ServiceVehicleOil: increaselevelEngineOilTank
            for (int engineOilTanksIndex = 0; engineOilTanksIndex < engineOilTanks.size(); engineOilTanksIndex++) {
                Method increaseLevelOfEngineOilTank = serviceVehicleOilPort.get(0).getClass().getDeclaredMethod("increaseLevelOfEngineOilTank", Object.class);
                LogEngine.instance.write("increaseLevelOfAPUOilTank = " + increaseLevelOfAPUOilTank);
                int increasedAmount = increaseLevelOfEngineOilTank.invoke(serviceVehicleOilPort.get(0), engineOilTanks.get(engineOilTanksIndex));
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
